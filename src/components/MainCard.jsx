...
export default function MainCard({ data }) {
  // Check if data or data.weather is undefined
  if (!data || !data.weather) {
    console.error("Invalid or missing data", data);
    return null; // Return null to prevent rendering if data is not available
  }
  ...
  if (weatherIconCode === "01d" || weatherIconCode === "01n") {
    ...
    console.warn("Unknown weather icon code:", weatherIconCode);
    weatherImage = clear_icon; // Set a default icon
  }
  ...
  return (
    <div className="card-container bg-gradient-to-b from-blue-800 to-blue-500 p-8 rounded-lg shadow-lg max-w-xs flex flex-col items-center justify-center">
      <h4 className="text-white text-2xl font-semibold mb-2 text-left">
        Current Weather
      </h4>
